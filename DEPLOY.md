...
3. **Cloudflare Pages** — New project → connect the repo. Build: `npm run build`. Output: `dist`. Or upload `dist/` / Wrangler Pages deploy after auth. Confirm the `*.pages.dev` URL loads. Optional: set build env `PUBLIC_GA_MEASUREMENT_ID` to a GA4 ID (`G-XXXXXXXX`) so the build injects gtag. Leave it empty for no Google snippet.
